...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      ekf_.x_(0) = rho * cos(phi); // initialize px with rho * cos phi
      ekf_.x_(1) = rho * sin(phi); // initialize py with rho * sin phi
      ekf_.x_(2) = rho_dot * cos(phi); // initialize vx with rho_dot * cos phi
      ekf_.x_(3) = rho_dot * sin(phi); // initialize vy with rho_dot * sin phi
    ...
    previous_timestamp_ = measurement_pack.timestamp_ / 1000000;
  ...
  dt = measurement_pack.timestamp_/ 1000000 - previous_timestamp_;
